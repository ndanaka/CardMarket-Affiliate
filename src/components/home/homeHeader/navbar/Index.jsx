...
const Navbar = () => {
    ...
    return (
        <>
            <nav ref={navItem} className="flex flex-wrap w-full bg-[#202938] h-12  min-[410px]:px-10 px-2">
                <NavItem label={'Homepage'} handle={() => {
                    navigate('/homepage');
                    setShowNav(t => ({ ...t, color: 'Homepage' }))
                }} />
                <div className='relative ' >
                    <NavItem label={'Links'} handle={() => setShowNav(t => ({ ...t, list: 'Links' }))} />
                    {showNav.list === 'Links' && <NavLinks />}
                </div>
                <div className='relative' >
                    <NavItem label={'Payments'} handle={() => setShowNav(t => ({ ...t, list: 'Payments' }))} />
